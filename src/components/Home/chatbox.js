...
const ChatRoom = () => {
  ...
  const { sendMessage, joinRoom, isConnected, getUserStatus } = ChatSocket({
    onNewMessage: (message) =>
      message.sender_id && message.message && message.createdAt
        ? setMessages((prev) => [...prev, message])
        : setError('Tin nhắn không hợp lệ'),
    onLoadMessages: (messages) =>
      Array.isArray(messages)
        ? setMessages(messages) && setOtherId(messages.find((m) => m.sender_id.id_user !== current.id_user)?.sender_id.id_user)
    ...
    onUserStatus: (userId, status) => {
      if (userId === otherId) {
        // Cập nhật giao diện khi trạng thái của other thay đổi
